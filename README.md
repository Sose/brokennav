...
* [`src/brokennav/`](src/brokennav/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/brokennav/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `brokennav` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/brokennav/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns brokennav.example
  (:require [brokennav.config :as config])
